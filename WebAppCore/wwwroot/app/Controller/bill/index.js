var BillController = function () {
    //load các danh sách dữ liệu ra sẳn
    var cachedObj = {
        products: [],
        colors: [],
        sizes: [],
    ...
    this.initialize = function () {
        //when đãm bảo các load chạy xong rồi ms done chạy tiếp
        $.when(loadBillStatus(),
            loadPaymentMethod(),// mổi hàm này đều phải trả về return khi trả về hết ms chạy tiếp
            loadColors(),
            loadSizes(),
            loadProducts())
            .done(function () {
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) {
            if ($('#frmMaintainance').valid()) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        structures.notify('Save order successful', 'success');
        ...
        $('#btnAddDetail').on('click', function () {
            ...
            $('#tbl-bill-details').append(render);//append html xoá củ chèn thêm cái ms
        });
